...
const ja: Catalog = {
  ...
  "alerts.channels.foot":
    "Kavka は発報のたびにこれらへ一度ずつ依頼し、再試行はしません。OS が実際に通知を表示したかどうかは Kavka には伝えられず、拒否した Webhook はここではなく診断ログに記録されます（「バージョン情報」でオンにしている場合）。つまり「オン」は Kavka が依頼するという意味であり、誰かに届いたという意味ではありません。",
  "alerts.channels.os.denied":
    "お使いの OS が Kavka の通知を拒否しているため、これが変わらないかぎりこのスイッチは何も届けられません。macOS では「システム設定 → 通知 → Kavka」、Windows では「設定 → システム → 通知 → Kavka」です。この設定を Kavka が代わりに変更することはできません。",
  "alerts.channels.os.confirmed":
    "このチャンネルを確認するため、Kavka がいま通知を 1 件送りました。代わりに OS が許可を求めてきた場合は、いまここで答えてください。そのダイアログこそが、最初の本物のアラートを飲み込んでしまうものです。どちらも表示されなかったなら、このデスクトップにはまだ何も届いていません。",
  "groups.list.foot":
